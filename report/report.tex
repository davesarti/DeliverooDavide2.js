...
backs off per predicate --- base \texttt{FAILED\_INTENTION\_}\allowbreak\texttt{RETRY\_MS}=1\,s,
doubling on each consecutive failure
(\texttt{FAILED\_INTENTION\_}\allowbreak\texttt{BACKOFF\_FACTOR}=2) up to
\texttt{FAILED\_INTENTION\_}\allowbreak\texttt{MAX\_RETRY\_MS}=8\,s --- so a persistently blocked
target stops retry-storming; the streak resets on the predicate's next success, or automatically
once it goes $2\times$ the cap (16\,s) without a further failure, so a resolved block does not
leave a stale escalated cooldown behind. Each pass of
\texttt{loop()}:
...
verdict with no message exchange; the loser's \texttt{camp} scores $-1$, handing off to delivery
or exploration as an ordinary cold pocket would. The verdict cannot flip mid-camp, since camping
freezes the camper's own score while the partner's can only grow.
