...
\author{Davide Senette \\ \small Universit\`a di Cagliari --- Corso di Autonomous Software Agents}
\date{17 June 2026}
...
\begin{abstract}
This report describes the architecture of a two-agent cooperative system for DeliverooJS,
a grid-world delivery game in which parcel rewards decay over time. The system pairs a
classical BDI agent, built on an explicit belief--desire--intention cycle with continuous
score-based intention revision, with an LLM-controlled agent that interprets natural-language
missions and drives the same BDI machinery through an internal tool-call loop. Both agents
share a structurally identical belief state and a persistent, LLM-synchronized rule set that
biases option scoring without ever making goals unreachable. Coordination between agents is
implemented as an asynchronous directive/status/signal protocol layered on top of the BDI's
ordinary intention queue, supporting handoffs, rendezvous and barrier synchronization. When
classical pathfinding cannot make progress because a crate obstructs the only route, the agent
falls back to a PDDL STRIPS planner that can push crates aside, executing the resulting plan as
a non-preemptable committed maneuver.
\end{abstract}
...
\node[subbox] (bdiloop) at (7.3,5.1) {IntentionRevision loop};
\node[subbox] (coordlisten) at (7.3,4.2) {Coordination listener};
...
(Section~\ref{sec:scoring}), so a candidate generated here can never diverge from what scoring
would accept.
...
A crate-pushing maneuver (\texttt{pushBatch}-inserted via the same batched path) is not a
distinct predicate: it is a recovery strategy invoked from inside \texttt{go\_to} when A*
detects that a crate is the sole obstacle, and is described in Section~\ref{sec:pddl}.
...
below is computed fresh from \texttt{bs} at scoring time, never cached across ticks beyond the
EMA in \texttt{distanceFactor}.
...
cycles win). Hotness and patience are spatial, not rate-based:
\begin{equation}
\text{patienceMs} = \operatorname{clamp}\!\big(1000\cdot(1.37^{\,n-3}-1),\,0,\,8000\big),
...
constants are \texttt{MIN\_TILES}=4 and \texttt{SATURATION\_TILES}=10). While hot, the camp loss
...
\centering
\begin{tikzpicture}[font=\footnotesize]
\node[box,text width=3.0cm] (belief) at (4,7.6) {Belief update};
\node[box,text width=3.0cm] (optgen) at (4,6.75) {Option generation};
\node[box,text width=3.0cm] (score) at (4,5.9) {Score computation};
\node[box,text width=3.0cm] (sort) at (4,5.05) {Sort intentions (hysteresis check)};
\node[box,text width=3.0cm] (best) at (4,4.2) {Best intention selected};
\node[box,text width=3.0cm] (exec) at (4,3.35) {Execute intention};
...
\begin{itemize}
  ...
  \item \textbf{Adaptive timeout.} $\text{timeout} = \max(\text{GO\_TO\_TIMEOUT\_MS},\ |\text{path}|\cdot\text{movementDurationMs}\cdot\text{SAFETY\_FACTOR})$,
  with \texttt{SAFETY\_FACTOR}=3, so short legs fail fast and long legs are not spuriously timed out.
...
\texttt{rendezvous\_with\_partner}, \texttt{wait\_for\_partner} & \texttt{observe\_environment},
\texttt{calculate}, every rule/navigation store tool, \texttt{direct\_partner}, \texttt{signal\_partner} \\
\bottomrule
\end{tabular}
...
Coordination & \texttt{rendezvous\_with\_partner}, \texttt{direct\_partner}, \texttt{signal\_partner}, \texttt{wait\_for\_partner} \\
...
onto the same predicates Table~\ref{tab:plans} describes (\texttt{go\_to}, \texttt{go\_near},
\texttt{pickup}$\to$\texttt{go\_pick\_up}, \texttt{putdown}$\to$\texttt{go\_drop\_off},
\texttt{wait}), so directive execution reuses the ordinary plan layer verbatim.
...
\paragraph{Handoff protocol.} The LLM agent has no put-down primitive away from a delivery tile,
so in a cross-delivery handoff the \emph{partner} is always the picker: \texttt{direct\_partner(pickup)}
with no coordinates lets the BDI self-select via \texttt{\#selectBestPickup()} (reusing
\texttt{intentionScore} so it never diverges from normal play); a \texttt{status(ok,picked)}
reply lets the LLM follow up with \texttt{direct\_partner(putdown, x, y)}, which drops the parcel
at an agreed tile and steps the BDI aside (\texttt{vacateAfterDrop()}) before its own
\texttt{status(ok)}; the LLM agent then collects and delivers it.
...
\node[note,text width=4.0cm,font=\scriptsize] at (2,3.0) {wait\_for\_partner(cid) resolves (or a buffered status is consumed)};
...
\node[decision,text width=2.0cm] (blocked) at (4.3,4.5) {Crate blocks path?};
\node[box,text width=2.5cm] (continue) at (1.1,3.2) {No: execute A* path normally};
\node[box,text width=2.9cm] (genpddl) at (7.3,3.2) {Yes: generate PDDL problem};
\node[box,text width=2.5cm] (planner) at (7.3,2.1) {Online planner (5\,s deadline)};
\node[decision,text width=1.9cm] (found) at (7.3,1.0) {Plan found?};
...
\paragraph{End-to-end example.} ``Bring me five parcels but avoid the rightmost delivery tile''
exercises every layer at once: a RULE clause (\texttt{forbid\_delivery\_tile}, \texttt{more=true})
followed by a TASK clause (\texttt{collect\_and\_deliver(parcels=5)}, Section~\ref{sec:llm}).
Option generation still proposes the forbidden tile (Section~\ref{sec:options}); it loses purely
because Eq.~\ref{eq:drop-score} floors its score to 0, so the embedded pickup/camp/deliver cycle
(Sections~\ref{sec:scoring}--\ref{sec:plans}) banks all five parcels elsewhere without the
constraint ever being special-cased outside the scoring formula.
