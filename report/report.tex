...
\raggedbottom
\makeatletter
% On a page made up entirely of floats, LaTeX's default \@fptop/\@fpsep/\@fpbot
% all carry infinite stretch (plus 1fil) to spread the floats evenly across
% the full page height -- \raggedbottom does not reach this glue. Replacing
% them with small fixed skips pulls the floats up to the top of the page and
% packs consecutive floats together, instead of leaving large uneven gaps.
\setlength{\@fptop}{0pt plus 1pt minus 1pt}
\setlength{\@fpsep}{8pt plus 1pt minus 1pt}
\setlength{\@fpbot}{0pt plus 1pt minus 1pt}
\makeatother
...
\begin{description}
  ...
  (\texttt{startAutonomousBDI}), identical to Agent B's, and additionally listens on the chat
  channel: each message is interpreted by an LLM and executed through a tool-call loop
  (Section~\ref{sec:llm}). The embedded BDI keeps harvesting with zero model calls while the LLM
  merely reads/queries state, and is paused only when a tool takes exclusive control of the
  agent's body (movement, delegated collection, rendezvous).
  \item[Agent B --- the BDI agent.] A pure reactive BDI agent with no LLM in its loop: it scores
  and executes intentions autonomously (Section~\ref{sec:scoring}) and can be \emph{preempted} by
  directives from Agent A (Section~\ref{sec:coordination}).
\end{description}

Both agents instantiate \texttt{createBeliefState()} (Section~\ref{sec:belief}) independently;
the two belief states are kept consistent by an explicit synchronization channel for persistent
rules, not by sharing memory. The design separates four recurring concerns:
...
\texttt{map} & \texttt{tiles}, \texttt{deliveryTiles}, \texttt{spawnTiles}, \texttt{pushableTiles} (type-5/``5!'' crate-receiving tiles), \texttt{deliveryDistanceMap}, \texttt{spawnDistanceMap} (BFS tables, Section~\ref{sec:plans}), \texttt{grid}. \\
...
\midrule
\texttt{stackSize} & Array of \texttt{\{mode, count, met, unmet\}}; compatible constraints (e.g.\ \texttt{at\_least 2}, \texttt{at\_most 5}) coexist; conflicts are resolved at write time. \\
...
\paragraph{Object permanence.} Sensing is local and intermittent: a parcel or agent leaving
observation range simply stops appearing in updates. Belief-update paths treat this absence as
negative evidence with a short time-to-live rather than as silence: an agent's last known
position (\texttt{AGENT\_MEMORY\_TTL\_MS}, 3\,s) feeds the race-discount and soft-obstacle
computations only briefly before being dropped, and the same TTL discipline governs the
avoided-tile memory used in navigation (Section~\ref{sec:plans}). The spawn-tile staleness heat
map driving \texttt{explore} (Table~\ref{tab:plans}) ticks once per sensing snapshot rather than
per move, so it keeps cooling/heating tiles even while the agent stands still (camping, parked on
a coordination wait) instead of freezing.
...
\texttt{agent.pushBatch(options)} filters out predicates already cooled-down or queued, inserts
the rest, and re-sorts the queue once regardless of batch size, avoiding $O(n)$ re-sorts for $n$
new options.

\texttt{optionsGeneration} returns immediately when \texttt{bs.coordination.active} is true, so
ordinary scored options never compete with or preempt the active coordination intention
...
failed-intention cooldown pool (predicate $\to$ \texttt{\{addedAtMs, cooldownMs\}}). The cooldown
is \emph{exponentially backed off} per predicate. The first failure waits only 1\,s
(\texttt{FAILED\_INTENTION\_RETRY\_MS}), because a failure is almost always transient congestion
--- a crossing agent at a choke --- that clears within a moment; but each consecutive failure of
the \emph{same} predicate doubles the wait, $1 \to 2 \to 4 \to 8$\,s
(cap \texttt{FAILED\_INTENTION\_MAX\_RETRY\_MS}), so a persistently blocked target stops
generating a retry storm, while the low cap keeps the agent responsive once the obstruction
clears. The failure streak resets on the predicate's next success, or automatically after
$2\times$ the cap (16\,s) without further failures, so a long-resolved block never leaves a stale
escalated cooldown behind. Each pass of \texttt{loop()}:
...
\texttt{intentionScore(predicate)} is the single authority on a predicate's worth, computed fresh
from \texttt{bs} at scoring time.
...
\text{sumBanked} &= \sum_{p\in P} \text{banked}_p \\
...
\label{eq:camp-score}
\end{equation}
which outranks a plain delivery of the same load (so the agent tops up first) but stays below
any real pickup (\(\text{totalReward}+\text{reward}_n\)), so a visible parcel is still grabbed
first.

\paragraph{Contested-pocket tie-break.} Camp validity also depends on the partner's position, via
a shared radius
\begin{equation}
\operatorname{campRadius}(bs) = \lceil \text{observationDistance} \rceil,
\end{equation}
falling back to \texttt{CAMP\_PATROL\_RADIUS}=5 tiles without a declared observation distance ---
reused verbatim by the camp anchor/patrol footprint (Section~\ref{sec:plans}) and the exploration
de-conflict below. When the partner is within this radius of the harvest anchor, only the agent
with the \emph{lower} game score keeps camping (ties broken by id), so both sides reach the same
verdict with no message exchange; the yielding, higher-scored agent's \texttt{camp} scores $-1$,
handing off to delivery or exploration exactly as an ordinary cold pocket would. The verdict
cannot flip mid-camp: camping banks nothing, so the camper's own score is frozen while the
partner's can only grow.
...
\texttt{FAILED\_INTENTION\_RETRY\_MS} & 1000\,ms & Base cooldown before a failed predicate is re-probed (Section~\ref{sec:scoring}). \\
\texttt{FAILED\_INTENTION\_BACKOFF\_FACTOR} & 2 & Per-streak doubling factor applied to the cooldown above. \\
\texttt{FAILED\_INTENTION\_MAX\_RETRY\_MS} & 8000\,ms & Cap on the backed-off cooldown. \\
\texttt{CAMP\_PATROL\_RADIUS} & 5 tiles & Fallback \(\operatorname{campRadius}(bs)\) when the server declares no observation distance. \\
\bottomrule
\end{tabular}
...
intention is abandoned immediately. This naturally protects deliveries in final approach: the
drop-off score \emph{peaks} right before the tile (route distance $\to 0$), so the interruption
bar is highest exactly where switching would waste the most progress. A
\texttt{committedManeuver} flag (set during a PDDL crate-push, Section~\ref{sec:pddl})
unconditionally disables preemption, since restarting a partial push from scratch would waste
the detour already taken.
...
\textbf{requeues} a fresh \texttt{Intention} for the same predicate to compete again next sort.
...
\centering
\begin{tikzpicture}[font=\footnotesize]
% The loop-back curve below has a control point well above the "Belief
% update" node (needed to shape the arc), which would otherwise inflate
% TikZ's computed bounding box and reserve a lot of unused vertical space
% above the diagram. Pin the box to the actual visible content instead.
\useasboundingbox (-0.4,-1.05) rectangle (8.7,8.9);
...
\texttt{go\_drop\_off x y} & \texttt{go\_to} then \texttt{putdown()}, which itself credits \texttt{bs.metrics.deliveredParcels} from the server ack on genuine delivery tiles --- so opportunistic and PDDL-driven drops are counted identically (a handoff drop on a plain tile banks nothing). \\
\texttt{explore} & \texttt{go\_to} on the least-recently-visited reachable spawn tile (\texttt{findCellsToExplore}, travel-cost/staleness weighted). \\
...
\begin{itemize}
  ...
  hard only within \texttt{SOFT\_OBSTACLE\_HARD\_RADIUS}=2 tiles of the start (they are
  ...
  costs extra (added to \texttt{BASE\_STEP\_COST}\allowbreak{}=1) but is never refused, so an LLM navigation
  ...
  \item \textbf{\texttt{avoidTiles}.} A tile that just refused a move is hard-blocked for
  \texttt{AVOID\_TENURE\_MOVE\_FACTOR}=3 move-durations, forcing the replanner off a proven choke;
  the memory expires quickly enough that a cleared corridor is retried while still in sensing
  range.
  \item \textbf{Opportunistic pickup and delivery.} \texttt{opportunisticActions()} runs after
  every step: it grabs a free, rule-eligible parcel on the just-entered tile (zero detour) and
  auto-banks carried parcels on a delivery tile, unless a stack rule is unmet or the tile is
  penalised --- suspended while \texttt{coordination.active}. Both are best-effort: a failed
  socket ack is swallowed rather than thrown, so a transient hiccup never aborts the walking
  intention.
  ...
  \item \textbf{Penalty-aware travel-cost map.} \texttt{travelCostMap()} runs a single Dijkstra
  pass (\texttt{pathfinding/dijkstra.js}) from the agent's position, giving $O(1)$
  penalty-inclusive reachability/cost lookups reused by \texttt{goNear}, \texttt{explore}'s
  ranking (\texttt{findCellsToExplore}) and the camp anchor's cluster-centroid choice
  (\texttt{clusterCenter}) --- replacing one A*/BFS probe per candidate. An unreachable candidate
  is dropped rather than scored as \texttt{NaN}; without a usable map, callers fall back to plain
  geometric distance.
  \item \textbf{Exploration/partner de-conflict.} \texttt{explore} demotes (never drops)
  spawn-tile candidates within the partner's $\operatorname{campRadius}(bs)$
  (Section~\ref{sec:scoring}) to the tail of the ranking, so the two agents spread over separate
  pockets while still falling back to a watched tile if nothing else is reachable.
...
messages are pushed through \texttt{createMissionQueue}, serializing mission handling to avoid
races between rapid messages.

Pausing is \emph{lazy}: the embedded BDI keeps harvesting until the first body-controlling tool
(Table~\ref{tab:bdi-pausing}) is about to execute, via \texttt{pauseBdiForMission()} (idempotent).
A pure rule/query mission therefore never interrupts ongoing play. \texttt{finally}
unconditionally calls \texttt{bdi.resume()} at mission end, \emph{unless} the agent was parked on
an external signal (\texttt{partnerParkedOn}, Section~\ref{sec:coordination}), in which case the
next mission turn must relay it first.
...
\begin{description}
  ...
  as motivation: it resolves to a durable RULE or an active TASK, never a no-op.
  \item[Rejection whitelist.] A \emph{closed} list of four grounds --- unresolved coordinate
  placeholder, explicit negative reward, violation of an active rule, or garbled text; everything
  else must be classified and acted on, and inventing further reasons (vagueness, missing
  specifics, future tense) is explicitly forbidden.
  ...
  when to use \texttt{rendezvous\_with\_}\allowbreak\texttt{partner} versus manual \texttt{direct\_partner} +
  ...
  mission$\to$tool-sequence pairs (rejections, factual queries, durable rules incl.\ compound and
  replacement cases, harvesting tasks, rendezvous, handoffs), each annotated with its reasoning.
\end{description}
...
\begin{enumerate}
  ...
  \emph{before} execution; a violation is fed back as a tool observation instead of running the
  disallowed action.
  ...
  \texttt{collect\_and\_deliver} via \texttt{toolResult.terminal}) end the mission immediately on
  success, using the tool's own confirmation text as the reply (saving a separate
  \texttt{final\_reply} round-trip), \emph{unless} the tool errored or \texttt{more:true} flags a
  remaining compound clause.
  \item \texttt{final\_reply} always ends the mission with an explicit message.
\end{enumerate}

Concretely, for ``collect three parcels and deliver them'': the LLM emits one
\texttt{collect\_and\_deliver(parcels=3)} call; the embedded BDI is unpaused and watches
\texttt{deliveredParcels} climb with zero further model calls until the target is reached (or the
time budget elapses), ending the mission in a single round-trip.
...
listener that pushes directives into \texttt{bs.coordination.queue}, flips \texttt{active}, and
calls \texttt{agent.preemptForCoordination()}; \texttt{\#runCoordination()} then drains the queue
one directive at a time via \texttt{coordToPredicate}, reusing the ordinary plan layer
(Table~\ref{tab:plans}) verbatim. \texttt{vacateAfterDrop()} steps the agent off its cell after a
\texttt{putdown} so the partner can occupy it to collect the handoff parcel. A \texttt{resume}
directive clears \texttt{coordination.active}; an idle TTL
(\texttt{COORD\_RESUME\_IDLE\_TTL\_MS}=15\,s) auto-resumes defensively if the LLM partner
disappears mid-directive.

\paragraph{Handoff protocol.} A cross-delivery handoff is driven end-to-end by one tool,
\texttt{handoff\_to\_partner}, instead of the model manually sequencing
\texttt{direct\_partner}/\texttt{wait\_for\_partner} calls: (1) it repeatedly issues
\texttt{direct\_partner(pickup)} with no coordinates so the BDI self-selects via
\texttt{\#selectBestPickup()} (reusing \texttt{intentionScore}, never diverging from normal
play), up to a target count or safety cap (\texttt{HANDOFF\_MAX\_PICKUPS}); (2)
\texttt{direct\_partner(go\_near)} brings the loaded BDI adjacent to the LLM agent; (3)
\texttt{direct\_partner(putdown)} drops at the BDI's cell, and \texttt{status(ok)} arrives only
after \texttt{vacateAfterDrop()} steps it off the tile; (4) the LLM agent walks onto the vacated
tile and retries the pickup; (5) by default it then delivers the load to the nearest delivery
tile and reports what was actually banked.
...
\texttt{(up/down/left/right ?from ?to)} & Directional adjacency between two existing tiles. \\
\texttt{(at-agent ?t)} & Current agent position. \\
\texttt{(parcel ?p)}, \texttt{(at-parcel ?p ?t)}, \texttt{(carrying ?p)}, \texttt{(delivered ?p)} & Parcel type guard, floor position, carried state, terminal state. \\
...
\textbf{Actions:} four \texttt{move-*} actions (precondition: the adjacent tile is free), and
four matching \texttt{push-*} actions that move the agent onto the crate's tile while sliding the
crate one step further onto a free \texttt{pushable} tile. Both collapse to the same primitive at
execution time: the server has no separate push command, so moving \emph{into} a crate with a
clear type-5 tile behind it pushes it automatically (\texttt{planExecutor.js}'s
\texttt{ACTION\_MAP} maps every \texttt{push-*} to \texttt{actions.move(direction)}).
...
caches the map's static part (tiles, adjacency, delivery/pushable sets), rebuilding only on a new
map; the dynamic part --- occupancy, positions, parcel/crate locations --- is always recomputed
fresh. Tiles are named \texttt{t\_\textless x\textgreater\_\textless y\textgreater}; parcel/crate
ids are hashed to a stable \texttt{[a-z0-9]} suffix (djb2 variant, \texttt{sanitize}) since PDDL
object names cannot contain arbitrary characters and the mapping never needs inverting.
...
\texttt{onlineSolver(domain, problem)} against a \texttt{SOLVER\_}\allowbreak\texttt{TIMEOUT\_MS}=5\,s deadline;
an empty result and a timeout/solver error both normalize to \texttt{null}, so the caller falls
back to ordinary A* either way. While a plan executes, \texttt{bs.committedManeuver=true}
disables hysteresis preemption (Eq.~\ref{eq:hysteresis}), since a crate push deliberately walks
\emph{away} from the target first (to get behind the crate) and would otherwise look like a
worse intention to the revision loop.

\begin{center}
\begin{tikzpicture}[font=\footnotesize, scale=0.46, every node/.style={scale=0.46}]
...
\end{tikzpicture}
\captionof{figure}{Crate blocking and PDDL recovery: \texttt{crateBlocksRoute()} double-checks
the block before paying for a solver call; a found plan is a committed maneuver immune to
preemption.}
