...
\texttt{direct\_partner}/\texttt{wait\_for\_partner} calls: that manual choreography repeatedly
deadlocked on the drop tile (a putdown aimed at the LLM agent's own cell, or at a tile the BDI
could not reach), so the prompt now forbids it explicitly. The tool: (1) repeatedly issues
...
\texttt{direct\_partner(putdown)} at the BDI's own (now-adjacent) cell --- a \emph{drop-at-feet}
tile that is always reachable by the carrier and never the LLM agent's own cell --- and waits for
\texttt{status(ok)}, sent only after \texttt{vacateAfterDrop()} has stepped the BDI off the tile;
(4) walks the LLM agent onto the vacated tile and retries the pickup a few times, detecting
success from the carried-count delta rather than the pickup call's return value (since
\texttt{opportunisticActions()} may already have auto-grabbed the parcels on arrival); (5) by
default delivers the collected load to the nearest delivery tile and reports the parcels
actually banked. Each step degrades to a clear terminal failure message (unreachable teammate,
nothing to hand off, drop failed, load not collected) rather than leaving the mission to stall.
